...
public:
	...
	void Uninit()override;//終了処理
	void Update()override;//更新処理
	bool GetPress(JOYKEY nKey);//プレス情報を取得
	bool GetTrigger(JOYKEY nKey);//トリガー情報を取得
	...
	float GetAngle();//スティックの角度取得
	bool Connection();//パッドが接続か判定
	bool GetStick();//スティックが入力されてるか判定
private:
